  ...
  var middleware = function(req, res, next) {
    ...
    if (req.headers && req.headers.authorization) {
      ...
      if (parts.length == 2) {
        ...
        return next(new UnauthorizedError(res, 'credentials_bad_format', { message: 'Format is Authorization: Bearer [token]' }));
      }
    } else {
      return next(new UnauthorizedError(res, 'credentials_required', { message: 'No Authorization header was found' }));
    }

    jwt.verify(token, options.secret, options, function(err, decoded) {
      if (err) return next(new UnauthorizedError(res, 'invalid_token', err));
